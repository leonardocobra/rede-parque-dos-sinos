...
// computeScoreDistribuicao depende de computaScore (lib/score.js).
...
// Computa a distribuição de níveis de maturidade digital (Bronze/Prata/Ouro)
// a partir da lista de profissionais com dados de perfil, serviços e itens.
// Função pura (sem I/O) para ser testável.
//
//   profissionais: [{ id, foto_url, descricao, instagram, experiencia, verificado,
//                     tem_google, tem_fotos_google, tem_outro_diretorio, instagram_ativo,
//                     tem_fotos_trabalho, link_na_bio, usa_whatsapp_business,
//                     fez_meta_ads, fez_google_ads,
//                     profissional_servicos: [{ profissional_itens: [] }] }]
//
// Retorna: { niveis: { Bronze, Prata, Ouro }, media, distribuicao }
//   distribuicao: [{ nome, total, pct, corBadge }] — para renderização direta
export function computeScoreDistribuicao(profissionais = [], { computaScore: cs } = {}) {
  // Aceita computaScore injetado (para testes) ou importa dinamicamente.
  // Em produção, o chamador passa a função; em testes, também.
  if (!cs) throw new Error("computaScore é obrigatório");

  const contagem = { Bronze: 0, Prata: 0, Ouro: 0 };
  let somaPontos = 0;

  for (const p of profissionais) {
    const auto = {
      temGoogle: !!p.tem_google,
      temFotosGoogle: !!p.tem_fotos_google,
      temOutroDiretorio: !!p.tem_outro_diretorio,
      instagramAtivo: !!p.instagram_ativo,
      temFotosTrabalho: !!p.tem_fotos_trabalho,
      linkNaBio: !!p.link_na_bio,
      usaWhatsappBusiness: !!p.usa_whatsapp_business,
      fezMetaAds: !!p.fez_meta_ads,
      fezGoogleAds: !!p.fez_google_ads,
    };
    const { pontos, nivel } = cs(p, auto);
  ...
  const total = profissionais.length;
  const pct = (n) => (total ? Math.round((n / total) * 100) : 0);
  const media = total ? Math.round(somaPontos / total) : 0;

  const COR_BADGE = {
    Bronze: "bg-[#C4873A] text-white",
    Prata: "bg-[#7A94A8] text-white",
    Ouro: "bg-[#C9A84C] text-white",
  };

  const distribuicao = ["Bronze", "Prata", "Ouro"].map((nome) => ({
    nome,
    total: contagem[nome],
    pct: pct(contagem[nome]),
    corBadge: COR_BADGE[nome],
  }));

  return { niveis: contagem, media, distribuicao };
}
...
// Tipos de evento de share (indicação + divulgação + pedido de avaliação).
// Ver lib/eventos.js para a lista canônica.
const ehShare = (tipo) =>
  tipo === "share_perfil" ||
  tipo === "share_pos_avaliacao" ||
  tipo === "share_pedir_avaliacao";

// Origens cuja primeira visita veio de uma indicação (link compartilhado).
// `compartilhado` é derivado do referrer (wa.me, telegram…); `whatsapp` vem do
// utm_source dos links de divulgação. Ver derivarOrigem/parseUtm em eventos.js.
const ORIGENS_INDICACAO = new Set(["compartilhado", "whatsapp"]);

// Série temporal contínua dos últimos `dias` dias terminando em `agora`,
// bucketizada por dia. Cada bucket: { dia, visitas, perfilViews, contatos, shares }.
// Dias sem evento entram zerados (linha contínua no gráfico). `shares` agrega os
// dois tipos de indicação — é dado de apoio ao bloco de referral, não plotado na
// linha de tráfego (volume baixíssimo seria esmagado pelas visitas).
function serieDiaria(eventos, dias, agora) {
  ...
  for (let i = dias - 1; i >= 0; i--) {
    ...
    const b = { dia: k, visitas: 0, perfilViews: 0, contatos: 0, shares: 0 };
    buckets.set(k, b);
    ordem.push(b);
  }
  for (const e of eventos) {
    ...
    else if (ehShare(e.tipo)) b.shares += 1;
  }
  return ordem;
}
...
export function computeAnalyticsEventos(eventos = [], { dias = 30, agora = new Date() } = {}) {
  ...
  // Referral (métrica-norte): indicações feitas e seu eco no funil.
  let shares = 0;
  const sharesPorTipo = { share_perfil: 0, share_pos_avaliacao: 0, share_pedir_avaliacao: 0 };
  const profsComShare = new Set(); // profissionais distintos com ≥1 share
  const canalShareMap = new Map(); // canal do share -> contagem
  const sessoesIndicacao = new Set(); // sessões cuja origem é uma indicação
  let contatosIndicacao = 0; // contatos vindos dessas sessões
  ...
  for (const e of eventos) {
    ...
    // Eco da indicação no tráfego: a visita chegou por um link compartilhado.
    const viaIndicacao = ORIGENS_INDICACAO.has(e.origem);
    if (viaIndicacao && s) sessoesIndicacao.add(s);
    ...
    if (e.tipo === "page_view") {
      ...
      if (viaIndicacao) contatosIndicacao += 1;
    } else if (ehShare(e.tipo)) {
      shares += 1;
      sharesPorTipo[e.tipo] += 1;
      if (e.profissional_id) profsComShare.add(e.profissional_id);
  ...
  // Quebra do share por canal (como foi compartilhado), maior primeiro.
  // Eventos antigos sem canal caem em "(desconhecido)" — não distorcem o total.
  const sharesPorCanal = [...canalShareMap.entries()]
    .map(([canal, total]) => ({ canal, total }))
    .sort((a, b) => b.total - a.total);

  // Bloco de referral. Números ABSOLUTOS de propósito: com N baixo, taxas de
  // conversão seriam enganosas. `visitasIndicacao`/`contatosIndicacao` leem o eco
  // da indicação no tráfego (sessões cuja origem é um link compartilhado) — é uma
  // leitura direcional do funil, não atribuição fechada share→contato (não há id
  // de indicação propagado do compartilhamento até o clique de contato).
  const referral = {
    shares,
    porTipo: sharesPorTipo, // { share_perfil, share_pos_avaliacao }
    profissionaisAlcancados: profsComShare.size, // numerador da métrica-norte
    porCanal: sharesPorCanal,
    visitasIndicacao: sessoesIndicacao.size,
    contatosIndicacao,
  };
