...
export function tituloPerfil(prof) {
  ...
  return `${prof.nome} — ${servicoPrimario(prof)} em ${CIDADE} · A Rede`;
}

// Meta description com serviço principal, localização e descrição própria.
export function descricaoPerfil(prof) {
  ...
  const base = `${prof.nome} — ${servicoPrimario(prof)} em ${local}.`;
