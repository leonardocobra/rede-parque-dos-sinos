...
// Monta o JSON-LD ItemList para páginas de catálogo/categoria.
// catalogUrl e profUrlBase devem ser URLs absolutas (via absUrl).
export function catalogoItemListJsonLd(profs, catalogUrl, profUrlBase) {
  if (!profs || profs.length === 0) return null;
  return {
    "@context": "https://schema.org",
    "@type": "ItemList",
    url: catalogUrl,
    itemListElement: profs.map((p, i) => ({
      "@type": "ListItem",
      position: i + 1,
      url: `${profUrlBase}/${p.id}`,
      name: nomeComServico(p.nome, servicoPrimario(p)),
    })),
  };
}
