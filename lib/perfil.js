...
// Monta o JSON-LD (schema.org) do perfil para dados estruturados → rich results
// na busca (estrelas quando há avaliações). Tipo LocalBusiness porque é o que o
// Google usa para exibir nota/contagem de avaliações de um prestador local.
// `stats` é o retorno de computeStats (ou null); `url` é a URL absoluta do perfil.
export function perfilJsonLd(prof, stats, url) {
  ...
  const ld = {
    ...
    knowsAbout: prof.servico || undefined,
    address: {
      "@type": "PostalAddress",
