...
describe("computeAnalyticsEventos", () => {
  ...
  describe("referral", () => {
    it("lista vazia retorna bloco zerado", () => {
      ...
      expect(r.porTipo).toEqual({ share_perfil: 0, share_pos_avaliacao: 0 });
    ...
    it("conta shares por tipo e profissionais distintos alcançados", () => {
      const r = computeAnalyticsEventos([
        { tipo: "share_perfil", canal: "nativo", profissional_id: "p1", sessao_id: "s1" },
        { tipo: "share_perfil", canal: "whatsapp", profissional_id: "p1", sessao_id: "s2" },
        { tipo: "share_pos_avaliacao", canal: "whatsapp", profissional_id: "p2", sessao_id: "s3" },
      ]).referral;
      expect(r.shares).toBe(3);
      expect(r.porTipo).toEqual({ share_perfil: 2, share_pos_avaliacao: 1 });
      expect(r.profissionaisAlcancados).toBe(2); // p1 e p2 (p1 não duplica)
    });

    it("quebra shares por canal, maior primeiro, com bucket (desconhecido)", () => {
      const r = computeAnalyticsEventos([
        { tipo: "share_perfil", canal: "whatsapp", profissional_id: "p1" },
        { tipo: "share_perfil", canal: "whatsapp", profissional_id: "p2" },
        { tipo: "share_perfil", canal: "nativo", profissional_id: "p3" },
        { tipo: "share_pos_avaliacao", profissional_id: "p4" }, // sem canal (evento antigo)
      ]).referral;
      expect(r.porCanal).toEqual([
        { canal: "whatsapp", total: 2 },
        { canal: "nativo", total: 1 },
