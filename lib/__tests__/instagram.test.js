...
describe("instagramHandle", () => {
  it("extrai o handle limpo de formatos variados", () => {
    expect(instagramHandle("@joao")).toBe("joao");
    expect(instagramHandle("https://www.instagram.com/joao/?hl=pt")).toBe("joao");
    expect(instagramHandle("  joao.silva ")).toBe("joao.silva");
  });

  it("retorna null para valores vazios ou inválidos", () => {
    expect(instagramHandle("")).toBeNull();
    expect(instagramHandle(null)).toBeNull();
    expect(instagramHandle("joão silva!")).toBeNull();
  });
});
