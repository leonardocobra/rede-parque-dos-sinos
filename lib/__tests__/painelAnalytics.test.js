...
describe("computaAnaliticasPerfil", () => {
  ...
  describe("referral", () => {
    it("zeros quando não há eventos de share ou indicação", () => {
      const r = computaAnaliticasPerfil([], { agora });
    ...
    it("conta os três tipos de share", () => {
      const eventos = [
        { tipo: "share_perfil", canal: "whatsapp", criado_em: ts(1) },
        { tipo: "share_pos_avaliacao", canal: "nativo", criado_em: ts(2) },
        { tipo: "share_pedir_avaliacao", canal: "link_copiado", criado_em: ts(3) },
      ];
      const r = computaAnaliticasPerfil(eventos, { agora });
      expect(r.referral.shares).toBe(3);
    });

    it("agrega sharesPorCanal e ordena por total desc", () => {
      const eventos = [
        { tipo: "share_perfil", canal: "whatsapp", criado_em: ts(1) },
        { tipo: "share_perfil", canal: "whatsapp", criado_em: ts(2) },
        { tipo: "share_perfil", canal: "link_copiado", criado_em: ts(3) },
      ];
      const r = computaAnaliticasPerfil(eventos, { agora });
      expect(r.referral.sharesPorCanal[0]).toEqual({ canal: "whatsapp", total: 2 });
      expect(r.referral.sharesPorCanal[1]).toEqual({ canal: "link_copiado", total: 1 });
    });

    it("canal null/undefined cai em '(desconhecido)'", () => {
      const eventos = [
        { tipo: "share_perfil", canal: null, criado_em: ts(1) },
        { tipo: "share_perfil", canal: undefined, criado_em: ts(1) },
      ];
      const r = computaAnaliticasPerfil(eventos, { agora });
      expect(r.referral.sharesPorCanal).toHaveLength(1);
      expect(r.referral.sharesPorCanal[0].canal).toBe("(desconhecido)");
      expect(r.referral.sharesPorCanal[0].total).toBe(2);
    });

    it("conta visitasIndicacao (profile_view via compartilhado ou whatsapp)", () => {
      const eventos = [
        { tipo: "profile_view", origem: "compartilhado", criado_em: ts(1) },
        { tipo: "profile_view", origem: "whatsapp", criado_em: ts(2) },
        { tipo: "profile_view", origem: "instagram", criado_em: ts(3) },
      ];
      const r = computaAnaliticasPerfil(eventos, { agora });
      expect(r.referral.visitasIndicacao).toBe(2);
      expect(r.perfilViews).toBe(3); // total não é afetado
    });

    it("conta contatosIndicacao (contact_click via compartilhado ou whatsapp)", () => {
      const eventos = [
        { tipo: "contact_click", origem: "whatsapp", criado_em: ts(1) },
        { tipo: "contact_click", origem: "direto", criado_em: ts(2) },
      ];
      const r = computaAnaliticasPerfil(eventos, { agora });
      expect(r.referral.contatosIndicacao).toBe(1);
      expect(r.contatos).toBe(2);
    });

    it("shares não entram em fontes nem em perfilViews/contatos", () => {
      const eventos = [
        { tipo: "share_perfil", canal: "nativo", criado_em: ts(1) },
        { tipo: "share_pos_avaliacao", canal: "link_copiado", criado_em: ts(2) },
      ];
      const r = computaAnaliticasPerfil(eventos, { agora });
      expect(r.perfilViews).toBe(0);
      expect(r.contatos).toBe(0);
