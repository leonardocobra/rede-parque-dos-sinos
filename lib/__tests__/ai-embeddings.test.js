...
// ---------------------------------------------------------------------------
// contextoServico — a bio só vale como contexto para quem tem 1 serviço
// ---------------------------------------------------------------------------

describe("contextoServico", () => {
  it("prefere a descrição do próprio serviço", () => {
    expect(
      contextoServico({
        servico: "Pintor",
    ...
    ).toBe("Pintura residencial");
  });

  it("usa a bio quando o profissional tem um único serviço", () => {
    expect(
      contextoServico({
        servico: "Azulejista e Pintor",
        descricao: null,
        bio: "Trabalho com pintura e acabamentos",
        totalServicos: 1,
      })
    ).toBe("Trabalho com pintura e acabamentos");
  });

  it("NÃO usa a bio quando o profissional tem mais de um serviço", () => {
    // Caso real: bio sobre pintura vazando para o serviço "Barbeiro".
    expect(
      contextoServico({
        servico: "Barbeiro",
        descricao: null,
        bio: "Trabalho com pintura e acabamentos com pisos",
        totalServicos: 2,
      })
    ).toBeNull();
  });

  it("com múltiplos serviços cai para a descrição gerada, não para a bio", () => {
    expect(
      contextoServico({
        servico: "Unhas em gel/manicure",
        descricao: null,
        bio: "Bio sobre outra coisa",
        totalServicos: 3,
      })
    ).toMatch(/manicure/i);
  });

  it("a descrição do serviço continua valendo mesmo com múltiplos serviços", () => {
    expect(
      contextoServico({
        servico: "Redes Sociais & Marketing",
        descricao: "Gestão de redes sociais",
        bio: "Bio genérica",
        totalServicos: 3,
      })
    ).toBe("Gestão de redes sociais");
  });

  it("ignora descrição só de espaços e cai para a bio", () => {
    expect(
      contextoServico({ servico: "Pedreiro", descricao: "   ", bio: "Alvenaria", totalServicos: 1 })
    ).toBe("Alvenaria");
  });

  it("retorna null quando não há descrição, bio elegível nem descrição gerada", () => {
    expect(
      contextoServico({ servico: "Doces Gourmet", descricao: null, bio: null, totalServicos: 1 })
    ).toBeNull();
  });

  it("assume 1 serviço quando totalServicos não é informado", () => {
    expect(contextoServico({ servico: "Pedreiro", descricao: null, bio: "Alvenaria" })).toBe(
      "Alvenaria"
    );
  });
});
...
describe("gerarEmbeddingsProfissional", () => {
    ...
    const sb = {
      from: vi.fn().mockImplementation((tabela) => {
        if (tabela === "profissional_servicos") {
          return {
            select: vi.fn().mockReturnValue({
              eq: vi.fn().mockResolvedValue({ data: servicos, error: null }),
            }),
            update,
          };
        }
  ...
  it("não injeta a bio quando o profissional tem mais de um serviço", async () => {
    const servicos = [
      { id: "s1", servico: "Azulejista e Pintor", descricao: null, embedding: null },
      { id: "s2", servico: "Barbeiro", descricao: null, embedding: null },
    ];
    const sb = {
      from: vi.fn().mockReturnValue({
        select: vi.fn().mockReturnValue({
          eq: vi.fn().mockResolvedValue({
            data: servicos.map((s) => ({
              ...s,
              profissionais: { descricao: "Trabalho com pintura" },
            })),
            error: null,
          }),
        }),
    ...
    await gerarEmbeddingsProfissional(sb, "prof-1");
    const enviados = JSON.parse(mockFetch.mock.calls[0][1].body).input;
    expect(enviados).toEqual(["Azulejista e Pintor", "Barbeiro"]);
    expect(enviados.join(" ")).not.toMatch(/pintura/);
  });
});
...
function mockBackfillSb(rows, { updateOk = true, erroQuery = null } = {}) {
  const updateEq = vi.fn().mockResolvedValue({ error: updateOk ? null : { message: "falhou" } });
  const update = vi.fn().mockReturnValue({ eq: updateEq });
  const order = vi.fn().mockResolvedValue({
    data: erroQuery ? null : rows,
    error: erroQuery,
  });
  const select = vi.fn().mockReturnValue({ order });
  return { from: vi.fn().mockReturnValue({ select, update }), _update: update };
}

function textosEnviados(chamada = 0) {
  return JSON.parse(mockFetch.mock.calls[chamada][1].body).input;
}
...
describe("backfillEmbeddings", () => {
  ...
  it("retorna zerado quando não há serviços", async () => {
    const sb = mockBackfillSb([]);
    expect(await backfillEmbeddings(sb)).toEqual({ processados: 0, erros: 0 });
    expect(mockFetch).not.toHaveBeenCalled();
  });

  it("propaga erro da query", async () => {
    const sb = mockBackfillSb([], { erroQuery: { message: "sem permissão" } });
    await expect(backfillEmbeddings(sb)).rejects.toThrow("sem permissão");
  });

  it("sem forcar, só processa os serviços sem embedding", async () => {
    mockFetch.mockResolvedValue(respostaVoyage(1));
    const sb = mockBackfillSb([
      {
        id: "s1",
    ...
    expect(await backfillEmbeddings(sb)).toEqual({ processados: 1, erros: 0 });
    expect(textosEnviados()).toEqual(["Doces Gourmet"]);
    expect(sb._update).toHaveBeenCalledTimes(1);
  });

  it("com forcar, reprocessa todos", async () => {
    mockFetch.mockResolvedValue(respostaVoyage(2));
    const sb = mockBackfillSb([
      {
        id: "s1",
    ...
    expect(await backfillEmbeddings(sb, { forcar: true })).toEqual({ processados: 2, erros: 0 });
    expect(sb._update).toHaveBeenCalledTimes(2);
  });

  it("a bio entra para quem tem 1 serviço e é barrada para quem tem 2", async () => {
    mockFetch.mockResolvedValue(respostaVoyage(3));
    const bioPintor = { descricao: "Trabalho com pintura e acabamentos" };
    const sb = mockBackfillSb([
      // profissional com 2 serviços e uma bio só sobre pintura
      {
        id: "s1",
        servico: "Azulejista e Pintor",
        descricao: null,
      ...
      // profissional com 1 serviço só — a bio descreve esse serviço
      {
        id: "s3",
        servico: "Carpintaria",
        descricao: null,
        embedding: null,
        profissional_id: "p2",
        profissionais: { descricao: "Especialista em telhados" },
      },
    ]);

    await backfillEmbeddings(sb, { forcar: true });
    expect(textosEnviados()).toEqual([
      "Azulejista e Pintor",
      "Barbeiro",
      "Carpintaria: Especialista em telhados",
    ]);
  });

  it("conta erros quando o Voyage falha no lote", async () => {
    mockFetch.mockResolvedValue(erroVoyage(500));
    const sb = mockBackfillSb([
      { id: "s1", servico: "Pedreiro", descricao: null, embedding: null, profissional_id: "p1" },
    ]);

    expect(await backfillEmbeddings(sb, { forcar: true })).toEqual({ processados: 0, erros: 1 });
